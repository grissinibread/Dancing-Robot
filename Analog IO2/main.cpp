...
void timer_ISR(){
    if (k < length){
        ...
        timeTicker.attach(&timer_ISR, 2 / beat[k++]);
    } else {
        speaker.write(0);
        k = 0;

        timeTicker.attach(&timer_ISR, beat[k]);
    }}
...
int main() {
    // start by considering the first music note
    k = 0;

    // sleep and wait for recurring interrupts
    timeTicker.attach(&timer_ISR, 0.1);
